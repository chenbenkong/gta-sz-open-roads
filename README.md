...
所有玩家可选项都集中在**设置面板**（表头「设置」按钮、`O` 键、或暂停菜单进入），
驾驶时画面上不再常驻任何选项控件。

| 分组 | 内容 |
|------|------|
| 画面 | 三张画质卡（极致 / 均衡 / 流畅）+ 自适应画质开关 |
| 渲染分辨率 | 50%–100% 滑块，可手动钉住 |
| 玩法 | 自由模式（默认开）：不推送任务与接单提示 |
| 界面元素 | 小地图 / 时速表 / 导航条 / 按键提示 / 屏幕提示 各自独立开关 |

- **自适应策略**：先掉特效，再掉像素。只有画质降到「流畅」后帧率仍不达标，
  才会动分辨率（下限 50%）——丢反射读作设置选择，丢清晰度读作游戏坏了。
- **渲染分辨率可低于预设下限**：手动钉住不会被 `minRenderScale` 否决，
  否则滑块根本拉不动。
- **自由模式是完全停止任务系统**（默认开）：不推送任何接单、目标与经验压力，该系统的每帧开销也一并归零；城市手账仍可主动接活，进度不丢。

小地图本身就是城市地图的紧凑视图，**点击它即打开完整地图**（`M` 键同样可用）。
小地图与时速表按作者标注的一半呈现。

**无门槛自由**：F 键随时上车/下车，没有距离、视线或落点限制（车旁被堵也会强制落位）；T 坦克 / B 飞机 / G 无人机随时切换，步行中同样生效。

## 渲染性能

帧预算优先保证流畅度，视觉按「先掉特效、再掉像素」的顺序退让：

| 手段 | 说明 |
|------|------|
| 自适应画质 | 90 帧窗口的 p80 帧时判定：>24 ms 降档，<12 ms 且稳定两窗才升档 |
| 自适应分辨率 | 「流畅」档仍超预算才动分辨率，下限 50%，恢复需帧宽裕且稳定 |
| 后处理精简 | 移除 bloom（4 个全/半分辨率 pass）：它在 SSAO2 + 半浮点 + MSAA 链上不稳定（整屏单色渐变），且只作用于太阳高光 |
| SSAO 跟随档位 | 「流畅」档真正卸载接触阴影管线，不再白付 G-buffer 与五次全屏采样 |
| 指针拾取旁路 | `skipPointerMovePicking`：转向时鼠标每动一下不再全场景拾取 |
| 免多余清屏 | `autoClear=false`：天球每帧全屏重绘，引擎色清空是纯超绘 |
| 静态网格冻结 | 全部城市网格冻结世界矩阵并停止包围盒同步 |
| 距离剔除 | 建筑块 / 地标 / 阴影投射者按相机距离启停（原有，随档位收紧或放宽） |
| 批处理 / 实例化 | 交通与植被走 thin instance，立面按视点流式换入换出（原有） |
| 纹理压缩 | 资产为 meshopt + 压缩纹理管线（原有） |


## 构建与验证

```bash
# 构建前务必清掉残留 node 进程，否则 esbuild 会卡住
pwsh scripts/kill-builders.ps1
node scripts/build-esbuild.mjs

# 内容哈希下只同步缺失文件（秒级），并自动改写 index.html 的入口引用
node scripts/sync-deploy.mjs

# 像素回归门：截图 3D 视口判颜色数/单色占比/亮度σ/梯度/全黑占比
node scripts/check-frame-content.mjs http://127.0.0.1:PORT/gta-sz-open-roads/

# 界面截图 + 尺寸校验 + 小地图点击校验
node scripts/shoot-ui.mjs http://127.0.0.1:PORT/gta-sz-open-roads/ output/ui
```
